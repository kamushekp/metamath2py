...
class PatchTheoremStateOp:
    ...
    def apply(self, target: TheoremState):
        if target is None:
            raise ValueError("Cannot apply theorem patch without a target TheoremState")
        ...
        if is_list_field:
            ...
            if self.operation == "insert":
                ...
                seq.pop(self.index)  # type: ignore[arg-type]
            elif self.operation == "replace":
                seq[self.index] = self.content  # type: ignore[index]
            return

        # Scalar fields
        if self.operation == "remove":
            setattr(target, self.field, None)
        elif self.operation == "replace":
