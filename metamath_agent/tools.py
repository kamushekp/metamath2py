...
class SearchTheoremsTool(Tool):
    """Saplings tool backed by TheoremSearchClient.search.

    Parameters schema mirrors an OpenAI function tool and supports:
    ...
    async def run(
        ...
        *,
        ...
        # Return compact dicts; callers can also print the formatted string.
        return [
            {
                "path": r.path,
                "score": r.score,
...
class VerifyProofTool(Tool):
    """Saplings tool that verifies a proof module given a file path.

    By default assumes modules live under metamath2py.proofs and derives the
    module name from the path. Returns a structured dict with verification
    details; the formatted output is a short human-readable summary.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.is_terminal = True  # Can terminate when success
    ...
    def _derive_module_name(path: str, default_package: str) -> str:
        ...
        if "/metamath2py/proofs/" in norm:
            ...
            return stem.replace("/", ".")
        # Fallback: treat filename stem as module
